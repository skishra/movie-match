...
const newMovieModel = require("../models/favoriteMoviesModel");

router.delete("/favoriteMovies/deleteById/:id", async (req, res) => {
  const { id: movieId } = req.params; // Retrieve movieId from URL params

  try {
    // Attempt to delete the favorite movie
    const deletedMovie = await favoriteMoviesModel.findByIdAndDelete(movieId);
